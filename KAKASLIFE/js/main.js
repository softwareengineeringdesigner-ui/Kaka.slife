...
const PRODUCTS = [
  {
    ...
    stockText: "Sold Out",
    isSoldOut: true, // 👈 Change to true to mark product as SOLD OUT
    freeTester: true,
    freeDelivery: true,
    isLive: true,
    rating: 5.0,
...
function renderProducts(filter) {
  ...
  syncOfferBannerStock();
}

/* SYNC OFFER BANNER STOCK - START */
function syncOfferBannerStock() {
  const elite = getProduct(1);
  if (!elite) return;
  const buyBtn = document.getElementById('offerBuyNowBtn');
  const cartBtn = document.getElementById('offerAddToCartBtn');
  const restockBtn = document.getElementById('offerRestockBtn');
  const stockBadge = document.getElementById('offerStockBadge');
  const stockPerk = document.getElementById('offerStockPerk');
  const bannerTag = document.getElementById('offerBannerTag');
  const imgBadge = document.getElementById('offerImgBadge');
  const miniStockCard = document.getElementById('miniOfferStockCard');
  const isSoldOut = Boolean(elite.isSoldOut || elite.stockStatus === 'sold-out' || elite.stockStatus === 'outofstock');

  if (buyBtn) {
    if (isSoldOut) {
      buyBtn.innerHTML = '<i class="bi bi-slash-circle"></i> Sold Out';
      buyBtn.disabled = true;
      buyBtn.classList.add('btn-sold-out');
      buyBtn.removeAttribute('onclick');
    } else {
      buyBtn.innerHTML = '<i class="bi bi-lightning-fill"></i> Order Now — Rs. 1,799';
      buyBtn.disabled = false;
      buyBtn.classList.remove('btn-sold-out');
      buyBtn.setAttribute('onclick', 'buyNow(1)');
    }
  }

  if (cartBtn) {
    cartBtn.style.display = isSoldOut ? 'none' : '';
  }

  if (restockBtn) {
    restockBtn.style.display = isSoldOut ? 'inline-flex' : 'none';
  }

  if (stockBadge) {
    if (isSoldOut) {
      stockBadge.className = 'stock-badge-soldout';
      stockBadge.innerHTML = '<i class="bi bi-x-circle-fill"></i> Sold Out';
    } else {
      stockBadge.className = 'stock-badge-live';
      stockBadge.innerHTML = '<span class="stock-pulse-dot"></span> Available Stock';
    }
  }

  if (stockPerk) {
    if (isSoldOut) {
      stockPerk.innerHTML = '<i class="bi bi-bell-fill text-gold"></i><span><strong>Restocking Soon</strong> — Batch 1 sold out. Batch 2 currently in maturation</span>';
    } else {
      stockPerk.innerHTML = '<i class="bi bi-box-seam-fill text-gold"></i><span><strong>Available Stock</strong> — Genuine sealed bottle, dispatched within 24 hours</span>';
    }
  }

  if (imgBadge) {
    if (isSoldOut) {
      imgBadge.innerHTML = '<span class="float-pct" style="font-size:0.92rem;line-height:1.2;">SOLD</span><span class="float-lbl">OUT</span>';
      imgBadge.style.background = 'linear-gradient(135deg, #b71c1c, #7f0000)';
    } else {
      imgBadge.innerHTML = '<span class="float-pct">23%</span><span class="float-lbl">OFF</span>';
      imgBadge.style.background = '';
    }
  }

  if (bannerTag) {
    if (isSoldOut) {
      bannerTag.innerHTML = '<i class="bi bi-slash-circle"></i> SOLD OUT &bull; RESTOCKING SOON';
      bannerTag.style.background = 'rgba(220, 53, 69, 0.15)';
      bannerTag.style.borderColor = 'rgba(220, 53, 69, 0.4)';
      bannerTag.style.color = '#ef5350';
    } else {
      bannerTag.innerHTML = '<i class="bi bi-fire"></i> 23% OFF &bull; LIMITED DEAL';
      bannerTag.style.background = '';
      bannerTag.style.borderColor = '';
  ...
  if (miniStockCard) {
    if (isSoldOut) {
      miniStockCard.innerHTML = `
        <div class="mini-offer-icon">⏳</div>
        <h4 class="mini-offer-title">Batch 1 Sold Out</h4>
        <p class="mini-offer-desc">ELITE has sold out due to immense demand! Next production batch is in maturation. WhatsApp us to pre-reserve.</p>
        <div class="discount-badge" style="background:#b71c1c;border:1px solid #ef5350;">Sold Out</div>
      `;
    } else {
      miniStockCard.innerHTML = `
        <div class="mini-offer-icon">📦</div>
        <h4 class="mini-offer-title">Available Stock</h4>
        <p class="mini-offer-desc">Freshly batched 50ml bottles in stock and ready to ship within 24 hours. Luxury presentation packaging with seal of authenticity.</p>
        <div class="discount-badge" style="background:#1f1610;border:1px solid #c9a84c;">In Stock</div>
      `;
    }
  }
}
/* SYNC OFFER BANNER STOCK - END */
...
function createProductCard(p, idx) {
  ...
  const isSoldOut = Boolean(p.isSoldOut || p.stockStatus === 'sold-out' || p.stockStatus === 'outofstock');

  if (p.isLive) {
    if (isSoldOut) {
      const priceHTML = `
        <div class="product-price-live">
          <span class="price-sale" style="color:var(--text-muted);">Rs. ${p.price.toLocaleString()}</span>
          <span class="price-original"><del>Rs. ${p.originalPrice.toLocaleString()}</del></span>
          <span class="badge-discount-tag" style="background:#555;border-color:#666;">SOLD OUT</span>
        </div>`;

      const badgeHTML = `<span class="product-badge badge-sold-out"><i class="bi bi-slash-circle"></i> SOLD OUT</span>`;

      return `
        <div class="col-lg-4 col-md-6 mb-4 reveal ${delay}">
          <div class="product-card product-card-live product-card-sold-out" id="product-card-${p.id}">
            <div class="product-img-wrap">
              <img src="${p.image}" alt="${p.name}" loading="lazy" style="opacity:0.85;">
              ${badgeHTML}
            ...
            <div class="product-info">
              <div class="d-flex justify-content-between align-items-center mb-2">
                ${genderHTML}
                <span class="stock-badge-soldout"><i class="bi bi-x-circle-fill"></i> Sold Out</span>
              </div>
              ...
              <div class="product-perks-row mb-2">
                <span class="perk-pill"><i class="bi bi-bell text-gold"></i> Restocking Soon</span>
              ...
              <div class="product-card-cta-group" style="display:flex;flex-direction:column;gap:8px;">
                <button class="btn-add-cart btn-sold-out w-100" disabled style="grid-column: 1 / -1;">
                  <i class="bi bi-slash-circle"></i> Sold Out
                </button>
                <a href="https://wa.me/923287004634?text=Assalam-o-Alaikum!%20Please%20notify%20me%20when%20${encodeURIComponent(p.name)}%20is%20back%20in%20stock." target="_blank" class="btn-outline-gold w-100" style="padding:8px 12px;font-size:0.8rem;display:flex;align-items:center;justify-content:center;gap:6px;text-decoration:none;border-radius:var(--radius-sm);">
                  <i class="bi bi-whatsapp text-success"></i> WhatsApp Restock Alert
                </a>
              </div>
            </div>
          </div>
...
function addToCart(productId, qty = 1) {
  ...
  if (p.isSoldOut || p.stockStatus === 'sold-out' || p.stockStatus === 'outofstock') {
    showToast('⚠️', 'Sold Out!', `${p.name} is currently out of stock. Contact our WhatsApp support for restock updates.`, 'toast-warning');
    return false;
  }
...
function openProductModal(productId) {
  ...
  if (p.isLive && !isSoldOut) {
  ...
  } else if (p.isLive && isSoldOut) {
    if (priceRow) {
      priceRow.innerHTML = `
        <div class="d-flex align-items-baseline gap-2 flex-wrap mb-2">
          <span class="price-sale" style="font-size:1.85rem;color:var(--text-muted);">Rs. ${p.price.toLocaleString()}</span>
          <span class="price-original" style="font-size:1.1rem;"><del>Rs. ${p.originalPrice.toLocaleString()}</del></span>
          <span class="badge-discount-tag" style="background:#555;border-color:#666;">OUT OF STOCK</span>
        </div>
        <div class="d-flex gap-2 flex-wrap mb-2">
          <span class="stock-badge-soldout"><i class="bi bi-x-circle-fill"></i> Sold Out</span>
          <span class="perk-pill"><i class="bi bi-bell text-gold"></i> Restocking Soon</span>
    ...
    if (launchNotice) {
      launchNotice.style.background = 'rgba(220, 53, 69, 0.08)';
      launchNotice.style.borderColor = 'rgba(220, 53, 69, 0.3)';
      launchNotice.style.color = '#dc3545';
      launchNotice.innerHTML = `
        <i class="bi bi-exclamation-octagon-fill" style="font-size:1.2rem;color:#dc3545;"></i>
        <div>
          <strong style="display:block;">Currently Out of Stock / Sold Out</strong>
          <span style="font-size:0.8rem;color:var(--text-medium);">This fragrance is temporarily sold out. Contact us on WhatsApp to pre-reserve from our next batch!</span>
        </div>`;
    }
    ...
    if (actionsContainer) {
      actionsContainer.innerHTML = `
        <button class="btn-outline-gold" data-bs-dismiss="modal">Close</button>
        <button class="btn-gold btn-sold-out" disabled id="modalAddToCartBtn">
          <i class="bi bi-slash-circle"></i> Sold Out
        </button>
        <a href="https://wa.me/923287004634?text=Assalam-o-Alaikum!%20Please%20notify%20me%20when%20${encodeURIComponent(p.name)}%20is%20back%20in%20stock." target="_blank" class="btn-outline-gold" style="padding:10px 18px;display:inline-flex;align-items:center;gap:6px;text-decoration:none;">
          <i class="bi bi-whatsapp text-success"></i> WhatsApp Restock Alert
        </a>`;
    }
  } else {
...
function placeOrder() {
  ...
  const hasAvailable = cart.some(i => {
    const p = getProduct(i.id);
    const isSoldOut = Boolean(p && (p.isSoldOut || p.stockStatus === 'sold-out' || p.stockStatus === 'outofstock'));
    return p && p.isLive && !isSoldOut;
  });
  if (!hasAvailable) {
    showToast('⚠️', 'Sold Out!', 'Items in your cart are currently sold out or out of stock.', 'toast-warning');
    return;
  }
